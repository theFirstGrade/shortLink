...
import org.zhenhaochen.shortlink.project.mq.producer.ShortLinkStatsSaveProducer;
...
public class ShortLinkServiceImpl extends ServiceImpl<ShortLinkMapper, ShortLinkDO> implements ShortLinkService {
    ...
    private final DomainWhiteListConfiguration domainWhiteListConfiguration;
    ...
    public ShortLinkBatchCreateRespDTO batchCreateShortLink(ShortLinkBatchCreateReqDTO requestParam) {
        ...
        List<String> describes = requestParam.getDescriptions();
        ...
        for (int i = 0; i < originUrls.size(); i++) {
            ...
            try {
                ...
                log.error("batch create short link failed，param：{}", originUrls.get(i));
            }
        }
    ...
    public void updateShortLink(ShortLinkUpdateReqDTO requestParam) {
        ...
        if (Objects.equals(hasShortLinkDO.getGid(), requestParam.getGid())) {
            ...
            if (!rLock.tryLock()) {
                throw new ServerException("short link is being access, please try it later");
            }
            try {
...
//                        .gid(requestParam.getGid())
//                        .build();
//                linkTodayStatsMapper.update(linkStatsTodayDO, statsTodayQueryWrapper);
                List<LinkStatsTodayDO> linkStatsTodayDOList = linkTodayStatsMapper.selectList(statsTodayQueryWrapper);
        ...
        if (!Objects.equals(hasShortLinkDO.getValidDateType(), requestParam.getValidDateType())
            || (!Objects.equals(hasShortLinkDO.getValidDate(), requestParam.getValidDate()) && Objects.equals(requestParam.getValidDateType(), VailDateTypeEnum.CUSTOM.getType()))) {
    ...
    private String getFavicon(String url) {
        try {
            ...
            throw new ClientException("can not get the favicon");
        }
        return null;
    }

    private void verificationWhitelist(String originUrl) {
        Boolean enable = domainWhiteListConfiguration.getEnable();
        if (enable == null || !enable) {
            return;
        }
        String domain = LinkUtil.extractDomain(originUrl);
        if (StrUtil.isBlank(domain)) {
            throw new ClientException("wrong format of short link");
        }
        List<String> details = domainWhiteListConfiguration.getDetails();
        if (!details.contains(domain)) {
            throw new ClientException("following domains are allowed: " + domainWhiteListConfiguration.getNames());
        }
    }
}
